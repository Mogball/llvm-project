...
void AbstractLattice::onUpdate(DataFlowSolver *solver) const {
...
ChangeResult PredecessorState::join(Operation *predecessor) {
  return predecessors.insert(predecessor) ? ChangeResult::Change
...
void AbstractSparseDataFlowAnalysis::visitOperation(Operation *op) {
  ...
  SmallVector<AbstractLattice *> resultLattices;
  ...
  for (Value result : op->getResults()) {
    AbstractLattice *resultLattice = getLatticeElement(result);
  ...
  SmallVector<const AbstractLattice *> operandLattices;
  operandLattices.reserve(op->getNumOperands());
  for (Value operand : op->getOperands()) {
    AbstractLattice *operandLattice = getLatticeElement(operand);
...
void AbstractSparseDataFlowAnalysis::visitBlock(Block *block) {
  ...
  SmallVector<AbstractLattice *> argLattices;
  argLattices.reserve(block->getNumArguments());
  bool allAtFixpoint = true;
  for (BlockArgument argument : block->getArguments()) {
    AbstractLattice *argLattice = getLatticeElement(argument);
...
void AbstractSparseDataFlowAnalysis::visitRegionSuccessors(
    ...
    Optional<unsigned> successorIndex, ArrayRef<AbstractLattice *> lattices) {
...
void AbstractSparseDataFlowAnalysis::markAllPessimisticFixpoint(
  ...
  for (AbstractLattice *lattice : lattices) {
    markPessimisticFixpoint(lattice);
  }
}

void AbstractSparseDataFlowAnalysis::join(AbstractLattice *lhs,
                                          const AbstractLattice &rhs) {
